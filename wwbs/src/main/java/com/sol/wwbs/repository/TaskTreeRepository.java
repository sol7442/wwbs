...
public interface TaskTreeRepository extends JpaRepository<TaskTree,Integer> {
	...
	void updateRootNull(TaskTree root);
	...
	void updateRootNull(TaskTree root, int left, int right);
	
	@Transactional
	@Modifying //delete t from _tree t where (t.root is null or t.root = 1) and t.lft >=14 and t.lft <=15
	@Query("delete from TaskTree t where (t.root is null or t.root = ?1) and t.left >= ?2 and t.right <= ?3")
	void delete(TaskTree root,int left, int right);
//
//	@Transactional
//	@Modifying //delete t from _tree t where (t.root is null or t.root = 1) and t.lft >=14 and t.lft <=15
//	@Query("update TaskTree t set t.name = ?1 where t.root = 1 and t.left >= ?2 and t.right <= ?3")
//	void rename(String name, int left, int right);
//
//	@Transactional
//	@Modifying 
//	@Query("delete from TaskTree t where t.name= ?1")
//	void deleteByName(String name);
//	
//	@Transactional
//	@Modifying 
//	@Query("delete from TaskTree t where t.left >= ?1 and t.right <= ?2")
